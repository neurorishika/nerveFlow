...
def I_ach(o,V):
    ...
    return np.sum(np.transpose((o_*(V-E_ach))*g_ach),1)
...
def I_fgaba(o,V):
    ...
    o_[ind] = o
    o_ = np.transpose(np.reshape(o_,(n_n,n_n)))
    return np.sum(np.transpose((o_*(V-E_fgaba))*g_fgaba),1)

def I_inj_t(t):
    print(int(t*100))
    return current_input.T[int(t*100),:]
...
def dAdt(X, t): # X is the state vector
    ...
    CmdV = np.concatenate([CmdV_p,CmdV_l],0)
    
    dV = (I_inj_t(t) + CmdV - I_K(V, n_K) - I_L(V) - I_KL(V) - I_ach(o_ach,V) - I_fgaba(o_fgaba,V)) / C_m
    
    T_ach = np.where(np.logical_and(np.greater(t,fire_t+t_delay),np.less(t,fire_t+t_max+t_delay)),A,[0.0]*len(A))
